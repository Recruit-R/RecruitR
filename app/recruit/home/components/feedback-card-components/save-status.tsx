...
export function SaveStatus() {
    ...
    return (
            !editable() ? (
                        <div className={"absolute top-0 right-0 mt-4 mr-7 p-1 rounded-full border bg-secondary/75"}>
                             <BsEye className={"w-6 h-6 stroke-{.5} fill-ring/50"}/>
                        </div>
                    )
                    :
                <div className={"absolute top-0 right-0 mt-4 mr-7 p-1 rounded-full border bg-primary/25"}>
                    {
                        saved
